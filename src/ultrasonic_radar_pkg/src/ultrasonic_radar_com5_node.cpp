...
class Ultrasonic_radar_com : public rclcpp::Node{
 public:
  ...
  explicit Ultrasonic_radar_com(std::string name) : Node(name){
    ...
    this->declare_parameter("Publish_topic_14", "/Sensor_msgs/UltrasonicRadar/COM14"); 
    this->declare_parameter("Publish_topic_15", "/Sensor_msgs/UltrasonicRadar/COM15"); 
    this->declare_parameter("Publish_topic_16", "/Sensor_msgs/UltrasonicRadar/COM16"); 

    /*获取参数*/
    this->get_parameter("baudrate_UltrasonicRadar", baudrate_UltrasonicRadar);
    this->get_parameter("UltrasonicRadar5_ComName", ComName);
    this->get_parameter("Publish_topic_14", Publish_topic_14);
    this->get_parameter("Publish_topic_15", Publish_topic_15);
    this->get_parameter("Publish_topic_16", Publish_topic_16);
    
    // 创建发布
    rawData_publisher_ = this->create_publisher<std_msgs::msg::String>("/Sensor_msgs/UltrasonicRadar/COM4_rawData", 10);
    ultrasonicRadarCOM_a_publisher_ = this->create_publisher<std_msgs::msg::Int16>(Publish_topic_14, 10);
    ultrasonicRadarCOM_b_publisher_ = this->create_publisher<std_msgs::msg::Int16>(Publish_topic_15, 10);
    ultrasonicRadarCOM_c_publisher_ = this->create_publisher<std_msgs::msg::Int16>(Publish_topic_16, 10);
    ...
    if(ComSerial.isOpen()){
      RCLCPP_INFO(this->get_logger(), "Port 5 is opened. Serial name : %s", ComName.c_str());
    }else{
      RCLCPP_ERROR(this->get_logger(), "Open Port 5 failed! Serial name : %s", ComName.c_str());}
 ...
 private:
  ...
  std::string ComName;
  std::string Publish_topic_14,Publish_topic_15,Publish_topic_16;
  ...
  void read_serial() {
    size_t n5 = ComSerial.available();
    /*第一个中转盒*/
    if(n5!=0){
      for(int_i=0;int_i<14;int_i++){
        buffer_uint8_t[int_i] = 0;
      }
      n5 = ComSerial.read(buffer_uint8_t, n5);
      int_n = int(n5);
      intCount = 0;
      origin_data.data = "n5: " ;
...
int main(int argc, char** argv) {
  ...
  /*创建对应节点的共享指针对象*/
  auto node = std::make_shared<Ultrasonic_radar_com>("ultrasonicRadarCom5_node");
  /* 运行节点，并检测退出信号*/
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
