...
/*驱动一台*/
class gnss_com : public rclcpp::Node{
 public:
  // 构造函数,有一个参数为节点名称
  explicit gnss_com(std::string name) : Node(name){
    // 创建发布
    rawData_publisher_ = this->create_publisher<std_msgs::msg::String>("/Sensor_msgs/GNSS/rawData", 10);
    Gpdop_publisher_ = this->create_publisher<sensor_driver_msgs::msg::GnssGpdop>("/Sensor_msgs/GNSS/GPDOP", 10);
    Gpfpd_publisher_ = this->create_publisher<sensor_driver_msgs::msg::GnssGpfpd>("/Sensor_msgs/GNSS/GPFPD", 10);
    Gpfpd_NavSatFix_publisher_ = this->create_publisher<sensor_msgs::msg::NavSatFix>("/Sensor_msgs/GNSS/NavSatFix_GPFPD", 10);
    Gpgga_publisher_ = this->create_publisher<sensor_driver_msgs::msg::GnssGpgga>("/Sensor_msgs/GNSS/GPGGA", 10);
    Gpgga_NavSatFix_publisher_ = this->create_publisher<sensor_msgs::msg::NavSatFix>("/Sensor_msgs/GNSS/NavSatFix_GPGGA", 10);
    Gpgst_publisher_ = this->create_publisher<sensor_driver_msgs::msg::GnssGpgst>("/Sensor_msgs/GNSS/GPGST", 10);
    Gphdt_publisher_ = this->create_publisher<sensor_driver_msgs::msg::GnssGphdt>("/Sensor_msgs/GNSS/GPHDT", 10);
    Gpzda_publisher_ = this->create_publisher<sensor_driver_msgs::msg::GnssGpzda>("/Sensor_msgs/GNSS/GPZDA", 10);
    Pashr_publisher_ = this->create_publisher<sensor_driver_msgs::msg::GnssPashr>("/Sensor_msgs/GNSS/PASHR", 10);
    
    /*声明参数*/
    this->declare_parameter("baudrate_INS", 115200);
    this->declare_parameter("INS_ComName", "/dev/ttyUSB0"); 
    this->declare_parameter("gnss_frame_id", "/gnss"); 
    
    /*获取参数*/
    this->get_parameter("baudrate_INS", baudrate_INS);
    this->get_parameter("INS_ComName", ComName);
    this->get_parameter("gnss_frame_id", gnss_frame_id);

    Gpdop_data.header.frame_id = gnss_frame_id;
    Gpdop_data.header.frame_id = gnss_frame_id;
    Gpfpd_data.header.frame_id = gnss_frame_id;
    Gpfpd_NavSatFix_data.header.frame_id = gnss_frame_id;
    Gpgga_data.header.frame_id = gnss_frame_id;
    Gpgga_NavSatFix_data.header.frame_id = gnss_frame_id;
    Gpgst_data.header.frame_id = gnss_frame_id;
    Gphdt_data.header.frame_id = gnss_frame_id;
    Gpzda_data.header.frame_id = gnss_frame_id;
    Pashr_data.header.frame_id = gnss_frame_id;
    ...
    timer_ = this->create_wall_timer(
        10ms, std::bind(&gnss_com::read_serial, this));
  }
  ...
  void read_serial() {
    ...
    while (size_type_i < str_buffer.length())
    {
      ...
      else
      {
        ...
        //把有效的数据给解析的函数以获取经纬度
        gnss_com::String_process(str_buffer.substr(size_type_i, end - size_type_i));
...
int main(int argc, char** argv) {
  ...
  /*创建对应节点的共享指针对象*/
  auto node = std::make_shared<gnss_com>("gnss_com_node");
  /* 运行节点，并检测退出信号*/
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
